...
public sealed class TTSManager
{
    ...
    public async Task<byte[]?> ConvertTextToSpeech(string speaker, string text)
    {
        ...
        if (string.IsNullOrWhiteSpace(_apiUrl) ||
            !Uri.TryCreate(_apiUrl, UriKind.Absolute, out _))
        {
            _sawmill.Warning("TTS API URL is empty or invalid");
            return null;
        }
        ...
        try
        {
            ...
            using var response = await _httpClient.PostAsJsonAsync(_apiUrl, body, cts.Token);
            ...
            if (json.Results is not { Count: > 0 } ||
                string.IsNullOrWhiteSpace(json.Results[0].Audio))
            {
                _sawmill.Error("TTS request returned empty audio data");
                RequestTimings.WithLabels("Error").Observe((DateTime.UtcNow - reqTime).TotalSeconds);
                return null;
            }

            var soundData = Convert.FromBase64String(json.Results[0].Audio);
